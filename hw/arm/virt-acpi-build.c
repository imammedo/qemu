...
build_iort(GArray *table_data, BIOSLinker *linker, VirtMachineState *vms)
{
    ...
    build_append_int_noprefix(table_data, 0, 1); /* Revision */
    build_append_int_noprefix(table_data, 0, 4); /* Reserved */
    build_append_int_noprefix(table_data, rc_mapping_count, 4); /* Number of ID mappings */
    /* Reference to ID Array */
    ...
    if (vms->iommu == VIRT_IOMMU_SMMUV3) {
        ...
        for (i = 0; i < smmu_idmaps->len; i++) {
            ...
            build_iort_id_mapping(table_data, range->input_base, range->id_count, smmu_offset);
        }

        /* bypassed RIDs connect to ITS group node directly: RC -> ITS */
        for (i = 0; i < its_idmaps->len; i++) {
            range = &g_array_index(its_idmaps, AcpiIortIdMapping, i);
            build_iort_id_mapping(table_data, range->input_base, range->id_count, iort_node_offset);
        }
    } else {
